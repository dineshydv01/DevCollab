...
async function fetchContributorCount(owner, repo) {
  try {
    ...
    // No Link header means there's only one page of results.
...
async function fetchFromGitHubApi(owner, repo) {
  ...
  if (res.status === 403) {
    // GitHub overloads 403 for both "rate limited" and "genuinely
    // forbidden" — the rate-limit headers are how you tell them apart.
...
/**
 * Public entry point. Returns cached data if fresh; otherwise attempts
 ...
 * this IS the "handle GitHub API failures gracefully" requirement,
 * not just a try/catch around the fetch.
 */
export async function getProjectGithubInfo(project) {
  ...
  const cacheIsFresh =
    project.githubCacheUpdatedAt && Date.now() - project.githubCacheUpdatedAt.getTime() < CACHE_TTL_MS;
